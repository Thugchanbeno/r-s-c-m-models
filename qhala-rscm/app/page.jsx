...
import { ArrowRight, Users, Zap, Cpu, BarChartBig } from "lucide-react";
...
const HomePage = () => {
  ...
  const accentColorAmber = "text-amber-500";

  const benefits = [
    {
      id: "ai-allocate",
      icon: <Cpu size={28} style={{ color: `rgb(${primaryColorRgb})` }} />,
    ...
    >
      <div className="absolute inset-0 z-0 pointer-events-none opacity-80 md:opacity-100">
        <div
          ...
          style={{ background: `rgba(${primaryColorRgb}, 0.35)` }}
        ></div>
        <div
      ...
      <main className="relative z-10 flex flex-grow flex-col items-center justify-center text-center px-4 py-8 md:py-12">
        <div className="max-w-3xl">
          ...
          >
            {qrscmFullName.split(" ").map((word, wordIndex) => (
              <span key={wordIndex} className="whitespace-nowrap mr-1">
                {word.split("").map((letter, index) => (
                  <span
                    ...
                    style={{
                      transform:
                        hoveredIndex === `${wordIndex}-${index}`
                          ? "translateY(-6px) scale(1.03)"
                          : "translateY(0) scale(1)",
                    }}
                    onMouseEnter={() =>
                      setHoveredIndex(`${wordIndex}-${index}`)
                    }
        ...
        <div className="mt-10 md:mt-12">
          <Button
            ...
            variant="primary"
            size="md"
            className="px-10 py-3 text-lg shadow-xl hover:shadow-2xl transform hover:scale-105 active:scale-100 group"
          >
            Sign in
            <ArrowRight
              size={20}
